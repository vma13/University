class Model
  ...
  # Общий метод поиска нескольких записей по запросу
  def Model.find(connection, query)
    res = []
  ...
  def Model.find_all(connection)
    find(connection, ["SELECT * FROM #{table_name()}"])
  end
  ...
  # Ищет все записи по значению заданной колонки
  def Model.find_by(connection, column, value)
    find(connection, ["SELECT * FROM #{table_name()} WHERE #{column} = ?", value])
  end

  # Ищет все записи, id которых попадает в диапазон
  def Model.find_range(connection, range)
    return nil if range.empty?
    find(connection, ["SELECT * FROM #{table_name()} WHERE id IN (#{range.map{|e|e.to_s}.join(',')})"])
  end
