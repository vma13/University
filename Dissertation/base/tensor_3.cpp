...
int main() {
  ...
  std::cout << M << std::endl;
  std::cout << N << std::endl;
  std::cout << Ay->size1 << std::endl;
  std::cout << Ay->size2 << std::endl;
...
gsl_matrix* matrix_t = gsl_matrix_alloc(Vy->size2, Vy->size1);
gsl_matrix_transpose_memcpy (matrix_t, Vy);
// gsl_matrix_print(matrix_t, matrix_t->size1, matrix_t->size2);

gsl_matrix* m_t = gsl_matrix_alloc(Vy->size1, Vy->size1);
gsl_blas_dgemm (CblasNoTrans, CblasNoTrans, 1.0, Vy, matrix_t, 0.0, m_t);
gsl_matrix_print(m_t, m_t->size1, m_t->size2);
